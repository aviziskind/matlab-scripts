function [idx, C, sumD, D] = wgtKmeans(X, k, wgts, varargin) 
...
%   $Revision: 1.1.10.1 $  $Date: 2010/03/16 00:15:00 $

if nargin < 3
    error('stats:kmeans:TooFewInputs','At least three input arguments required.');
end
...
for rep = 1:reps
    ...
    try % catch empty cluster errors and move on to next rep
        ...
        nonempties = find(m>0);
        D(:,nonempties) = distfun(X, C(nonempties,:), distance, iter, wgts);
        d = D((idx-1)*n + (1:n)');
...
while true
    ...
    [C(changed,:), m(changed)] = gcentroids(X, idx, changed, distance, wgts); 
    D(:,changed) = distfun(X, C(changed,:), distance, iter, wgts);
    ...
    if ~isempty(empties)
        ...
        case 'singleton'
            ...
            for i = empties
                ...
                m(i) = 1;
                idx(lonely) = i;
                D(:,i) = distfun(X, C(i,:), distance, iter, wgts);

                % Update clusters from which points are taken
                [C(from,:), m(from)] = gcentroids(X, idx, from, distance, wgts);
                D(:,from) = distfun(X, C(from,:), distance, iter, wgts);
...
function D = distfun(X, C, dist, iter, wgts)
...
function [centroids, counts] = gcentroids(X, index, clusts, dist, wgts)
...
for i = 1:num
    ...
    if any(members)
        ...
        case 'sqeuclidean'
            if ~isempty(wgts)
                cur_wgts = wgts(members);
                cur_wgts = cur_wgts/sum(cur_wgts);
                centroids(i,:) = sum( bsxfun(@times, X(members,:), cur_wgts(:)), 1 );
            else
