function [coef, pval, tstat] = myCorr(x, y, varargin)
...
%   for the corresponding element of RHO.  If PVAL is small, say less
%   than 0.05, then the correlation RHO is significantly different
%   from zero.
%
...
%                   missing values, or 'pairwise' to compute RHO using
...
%   $Revision: 1.1.6.7 $  $Date: 2005/05/31 16:44:25 $
...
%   the two-tailed test by doubling the smaller of the one-tailed p-values.
    ...
    % differences between this version and the original 'corr.m'
    % (1) Also returns 't-stat'.
    % (2) In this version we assume we are taking correlation
    %   between x and y (and not an autocorrelation), 
    % (3) we also assume x and y are single vectors, not matrices.
    % (4) does a simple polynomial interpolation for 'blind spot' p values
    % with 360 degrees of freedom.
    % (5) prevents negative p-values for kendall's tau.
    ...
    pnames = {'type'  'tail'};
    dflts  = {'p'     'both'};
    [errid,errmsg,type,tail] = processVarargin(pnames,dflts,varargin{:});
    if ~isempty(errid)
        error(sprintf('stats:corr:%s',errid),errmsg);
    end
    
    % (( skip validating rows parameter [we assume there are no nan's in x & y]))
    
    % Can't do anything without at least two observations.
    if n < 2
        coef = NaN(1,1,outClass);
        pval = NaN(1,1,outClass);
        tstat = NaN(1,1,outClass);
        return;
    end
    n2const = n*(n-1) ./ 2;
    n3const = (n+1)*n*(n-1) ./ 3;
    ...
    coef = zeros(1,1,outClass);
    if nargout > 1
        pval = zeros(1,1,outClass);
    end
    ...
    case 'p' % Pearson's linear correlation
        x0 = xi - mean(xi);
        y0 = yj - mean(yj);
        coef = (x0./norm(x0))' * (y0 ./ norm(y0));

        if nargout > 1
            [pval, tstat] = pvalPearson(tail, coef, n);
        end
    ...
    case 'k' % Kendall's tau
        ...
        coef = K ./ sqrt((n2const - xadj(1)).*(n2const - yadj(1)));
        ...
        if nargout > 1
            ...
            if ties
                ...
                [pval, tstat] = pvalKendall(tail, K, stdK, xrank, yrank);
            else
                stdK = sqrt(n2const*(2*n+5)./9);
                [pval, tstat] = pvalKendall(tail, K, stdK, n);                
            end
            pval = max(pval, 0); %%%%%%
        end
    ...
    case 's' % Spearman's rank correlation
        ...
        if nargout > 1
            ...
            % Otherwise, we'll compute it with the rest, later.
            if ties
                [pval, tstat] = pvalSpearman(tail, D, meanD, stdD, xrank, yrank);
            else
                meanNoTies = n3const ./ 2;
                stdNoTies = n3const ./ (2*sqrt(n-1));
                [pval, tstat] = pvalSpearman(tail, D, meanNoTies, stdNoTies, n);
            end
        end
    end
        
    coef(coef > 1) = 1; coef(coef < -1) = -1; % min/max drops NaNs


    %--------------------------------------------------------------------------

    function [p, tstat] = pvalPearson(tail, rho, n)
        ...
        case 'b' % 'both or 'ne'
            p = 2*tcdf(-abs(t),n-2);
        case 'r' % 'right' or 'gt'
            if n == 360 && ( t > 8.8 && t < 18.9300)  % fill in blind spot (where p == 0) // (note: i've now put this directly into tcdf)   
                poly_approx = [-0.00385, 0.2135, 0.248, 0.303];
                p = 10.^(-polyval(poly_approx, double(t)));
            else
                p = tcdf(-double(t),n-2);
            end
            if (p == 0) && (t <= 148) && (n == 360)
    ...
    function [p, tstat] = pvalKendall(tail, K, stdK, arg1, arg2)
    ...
    function [p, tstat] = pvalSpearman(tail, D, meanD, stdD, arg1, arg2)
