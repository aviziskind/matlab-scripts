...
void spear(float data1[], float data2[], unsigned long n,
     ...
     // zero as probrs. The external routines crank(below) and qsort(x8.2) are used. A small value
    ...
    float erfcc(float x);
    void qsort(unsigned long n, float arr[], float brr[]);
    unsigned long j;
...
*/
    ...
    wksp1=vector((long)1, l);
    wksp2=vector((long)1, l);
...
*/
    ...
    qsort(n, wksp1, wksp2); // Sort each of the data arrays, and convert the entries to
    crank(n, wksp1, &sf);   // ranks. The values sf and sg return the sums Sum(f_k^3 - f_k)
    qsort(n, wksp2, wksp1); // and Sum(g_m^3 - gm), respectively.
    crank(n, wksp2, &sg);
    *d=0.0;
    ...
    *rs=(1.0-(6.0/en3n)*(*d+(sf+sg)/12.0))/sqrt(fac); //Rank correlation coefficient,
    ...
    free_vector(wksp2, (long)1, l);
    free_vector(wksp1, (long)1, l);
