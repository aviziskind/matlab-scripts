...
%Adjust input
if length(siz)<=length(sub)-1
    %Adjust for trailing singleton dimensions
    siz = [siz ones(1,length(sub)-length(siz)-1)];
else
    %Adjust for linear indexing on last element
    siz = [siz(1:length(sub)-2) prod(siz(length(sub)-1:end))];
end

%Compute linear indices
k = [1 cumprod(siz(1:end-1))];
ndx = 1;
s = size(sub(1)); %For size comparison
for i = 1:length(siz),
    v = sub(i);
    %%Input checking
    if ~isequal(s,size(v))
